...
class CodexStatusTest(unittest.TestCase):
    def test_collects_and_renders_a_session(self) -> None:
        ...
        self.assertIn("3\t1\t145\t0\tcjk_heading\tKindleDisplay", layout)
        self.assertIn("2\t1\t183\t8\tcjk_row\t阅读 wi.", layout)
        self.assertIn("         gpt-test       R  20%      1k   50/80", layout)
        self.assertIn("阅读 wi.", layout)
